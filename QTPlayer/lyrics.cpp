...
void Lyrics::parseLyricsFromMusicId(int musicId, QMap<qint64, int> &lyric2IdxMap, LyricModel *lyricModel)
{
    ...
    Lyrics::parsePic(musicId);
}

void Lyrics::parsePic(int musicId)
{
    HttpUtil *httpUtil = new HttpUtil{};
    connect(httpUtil, &HttpUtil::parseJson, [=](const QJsonObject &totalObject){
        // 列出json里所有的key
        QJsonArray songsArray = totalObject["songs"].toArray();
        if (!songsArray.isEmpty()) {
            QJsonObject firstSong = songsArray[0].toObject();
            QJsonObject alObject = firstSong["al"].toObject();
            QString Url = alObject["picUrl"].toString();
            Lyrics::m_picUrl=Url;
        }
    });
    httpUtil->getPicRequest(musicId);
}
